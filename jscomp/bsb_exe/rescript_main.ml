  ...
  | _ ->
      let config_opt =
        Bsb_ninja_regen.regenerate_ninja ~package_kind:Toplevel
          ~per_proj_dir:Bsb_global_paths.cwd ~forced:!force_regenerate ~warn_legacy_manifest:false
      in
      ...
      if !list_files then
        match
          Bsb_ninja_regen.regenerate_ninja ~package_kind:Toplevel ~forced:true
            ~per_proj_dir:Bsb_global_paths.cwd ~warn_legacy_manifest:false
        with
        | None -> assert false
    ...
    if argv_len = 1 then (
      ...
      let config_opt =
        Bsb_ninja_regen.regenerate_ninja ~package_kind:Toplevel ~forced:false
          ~per_proj_dir:Bsb_global_paths.cwd ~warn_legacy_manifest:true
      in
