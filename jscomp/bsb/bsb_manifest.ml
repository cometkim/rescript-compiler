...
let from_json (json : Ext_json_types.t) ~(filename : string) : Bsb_manifest_types.t =
  ...
  | Obj { map } -> (
    ...
    let package_specs = extract_package_specs map ~suffix in
    {
      ...
      bs_dependencies = extract_string_list map Bsb_build_schemas.bs_dependencies;
      bs_dev_dependencies = extract_string_list map Bsb_build_schemas.bs_dev_dependencies;
